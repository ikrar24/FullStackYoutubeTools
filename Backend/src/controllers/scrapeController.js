...
import analysisSeo from '../utils/analysisSeo.js';


export async function scrapeYouTubeData(req, res) {
  try {
    const { url } = req.query;
    ...
    // analysisSeo 
  const seoAnalysis = await analysisSeo(response)
  // console.log(seoAnalysis);
  



    res.json({user:response , analysis:seoAnalysis });
  } catch (err) {
